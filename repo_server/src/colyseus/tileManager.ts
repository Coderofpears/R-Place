...
export default class TileManager {
    ...
    placeTile(x: number, y: number, terrain: string, collides: boolean, depth: number, player?: any) {
        if(this.tiles[`${x}_${y}`]) return false;
        if(player && !this.room.canCreativeEdit(player, "place or edit the map")) return false;

        let info: TileInfo = { terrain, depth, collides };
        this.tiles[`${x}_${y}`] = info;
        this.map.tiles[`${x}_${y}`] = { terrain, depth, collides };
    ...
    removeTile(x: number, y: number, depth: number, player?: any) {
