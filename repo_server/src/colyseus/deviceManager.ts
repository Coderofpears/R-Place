...
export default class DeviceManager {
    ...
    createDevice<T extends keyof DeviceOptions = any>(info: DeviceInfo<T>, init = false, player?: Player) {
        if(player && !this.room.canCreativeEdit(player, "place or edit the map")) return null;
        ...
        if(init || this.initialized) device.init?.();
    ...
    removeDevice(device: BaseDevice, player?: Player) {
        if(player && !this.room.canCreativeEdit(player, "delete map changes")) return false;
    ...
    hydrateSavedDeviceState() {
        for(let savedDevice of this.map.devices as any[]) {
            let device = this.getById(savedDevice.id);
            if(!device) continue;

            if(savedDevice.globalState) {
                device.globalState = structuredClone(savedDevice.globalState);
            }

            if(savedDevice.teamStates) {
                device.teamStates = structuredClone(savedDevice.teamStates);
            }

            if(savedDevice.playerStates) {
                device.playerStates = structuredClone(savedDevice.playerStates);
            }

            if(savedDevice.codeGrids) {
                device.codeGrids = Object.values(savedDevice.codeGrids);
            }
        }
    }
    ...
    broadcastWireChanges(addedWires: MapInfo["wires"], removedWires: string[]) {
        this.room.broadcast("WORLD_CHANGES", {
            wires: {
                addedWires,
                initial: false,
                removedWires
