...
export default class Inventory {
    ...
    onConsume(message: { x: number, y: number }) {
        ...
        if(!this.room.canCreativeEdit(this.player, "place or edit the map")) return;
        ...
        if(!this.removeItemSlot(1, this.inventory.activeInteractiveSlot)) return;

        this.room.terrain.placeTile(message.x, message.y, item.terrainId, true, 6, this.player);
    }
}
